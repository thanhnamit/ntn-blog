...
If distributed tracing brings visibility into the lifetime of a request, metrics are important for monitoring and alerting overall system health. Mission critical systems continuously emit telemetry data (i.e: gauges, counter and numbers) in time-series format so that it can be collected, statistically transformed, correlated and visualised to predict potential issues. Telemetry has been long used in [other industries](https://en.wikipedia.org/wiki/Telemetry). For example, most of cars today has OBD (on board diagnostic) system that you can connect to with a scanner to read [its trouble code](https://en.wikipedia.org/wiki/OBD-II_PIDs); a solar monitoring system can report metrics about energy consumption, production, grid export and import.

In software world, engineers have utilised various tools to monitor performance of software systems, just to name a few:
...
This complexity in the tooling landscape brings many challenges for business to build and operate their platform in an unified way. With the vendor-neutral approach, OTel does not reinvent a new metric-based library or platform, instead its direction is on standardising metric types, telemetry pipeline architecture and providing a vendor-agnostic protocol for the tramission of telemetry data over network. While still in beta, the Metrics API and Go SDK are ready for some certain experiments, we will tap into the key concepts by examples.
...
- Metrics API (the user-facing API) captures runtime measurements about a program execution. Application developer will pass raw measurements with metadata to the unified interface.
...
- The instrument invokes SDK implementation api and pass through the metric event.
...
- Processor picks a suitable aggregator to perform aggregation logic on the metric events.
- The Controller manages the how and when the metric collection happen via collection mode (`push` or `pull`), collection checkpoints and the timer.
- Exporters export outcome to storage backend or scraper via configured metric protocol (i.e OTLP).

In the [ShortenIt example](https://thanhnamit.netlify.app/2021/01/03/applying-observability-with-opentelemetry-part-1-distributed-tracing/#explore-otel-examples-with-shortenit), we will explore how OpenTel export simple metrics to Prometheus backend and view it via Grafana. The code below starts a metric server and expose a http endpoint `/metrics` for Prometheus scraper. It is also configured with auto-instrumentation for application runtime and host, this is very handy if we don't want to write code to collect those metrics from scratch.
...
For the SDK to apply proper processing logic to each type of measurement, the metric event must carry its own semantics (or meaning). OTel define three key semantic groups:
...
For each measurement, the meter provides a corresponding instrument. Let's loop throuch each of them with some examples.

### Counter

Counter can be used when it is meaningful to record sum of a metric at a point in time. This instrument has following characteristics:

- Synchronous: size of a request is captured immediately during execution.
- Adding (measurement): only single value (total of bytes) makes sense.
...
For exporting, OTel metric values need to be converted to vendor's format. For example, Prometheus only accepts certain metric types such
as Counter, Gauge, Histogram and Summary. This graph show the rate of increment of total request size processed by the API.
...
The common use of ValueRecorder instrument is to record every single latency measurement. It is useful to gain a statistical view 
...
When plot a graph on Grafana we have
...
Output from metric endpoint
```
# TYPE runtime_go_gc_count counter
runtime_go_gc_count 463
```

As it is monotonic and adding, we can graph an increase rate:
...
<https://pkg.go.dev/go.opentelemetry.io/contrib/instrumentation/runtime> contains several example of this instrument to measure:
...
This is an asynchronous version of the ValueRecorder which collects grouping measurements non-monotonically and produce distribution of recorded
values (i.e CPU temperature)

### How to pick the right instrument

Using the right instrument would require library users to have some understanding of the semantics. Instead of strolling through the spec, I have created this decision tree to help making such decision easier.

{{< figure src="/img/posts/2021-02-10-instrument-decision.png" >}}

### A quick note about cardinality

On a [retrospective post](https://thenewstack.io/observability-a-3-year-retrospective/) about Observability, Charity Major (CTO of HoneyComb.io) stressed the importance of high-cardinality data for observable systems. Typical metrics-based tools (were designed for monolithic systems) only provide aggregated low-cardinality dimensions (metrics values associated with node name, container id, build number...) which are not useful for debugging purpose in context of distributed systems. To help uncover hypotheses and pin down exact the bad code block, high-cardinality data must be available (unique IDs such as request ID, customer ID or event ID...) along with distributed context and raw events.

OTel library plans to support both cumulative (as demonstrated with Prometheus in this post) and stateless exporters to output high-cardinality events to downstream collector or metric backend. This has been discussed with details  in [a community talk by Josh MacDonald](https://youtu.be/L-Ss8PtWlRA?t=1951).

## Logs

At current stage, OTel does not provide a new Logging API. Instead, it relies on current well known logging libraries to supply log events in existing formats. Log entries are forwarded to a OTel collector which performs transformation to [a log data model](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/logs/data-model.md) before exporting to backend. The log data model hence is defined to share common understanding of what log entries should look like, including the key information for log correlation (time of execution, execution context and resource context). We will explore this area more in next post.

## Summary

In the first two posts, we have looked into each individual component of OpenTelemetry - tracing, . This however doesn't illustrate big picture as well as what improvement OTel brings to the play. In the next post, I will attempt to brings this together by combining traces, metrics and logs. I also will discuss possible deployment topologies for microservices architecture within a cloud native platform such as AWS. Stay tune!
